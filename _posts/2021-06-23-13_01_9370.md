...
```
코드를 보면 알 수 있듯이 g, h, s 각각에 대해 모든 정점에 대한 최단 경로를 서로 다른 dist 배열에 저장시켜주었다. 최종적으로 모든 destination 후보들에 대해서 `s -> g -> h -> d` 와 `s -> h -> g -> d` 최단거리의 최솟값과 `s -> d` 경로의 최단 거리를 비교해주어서 일치할 시 정답에 추가시켜준다.  
