...
```
레벨을 매겨주는 bfs 함수이고 동작은 앞서 했던 에드몬드의 bfs와 비슷하다. 유량이 흐를 수 있는 경로가 존재할 시 parent가 아닌 level함수를 이전 level에 1씩 증가시키면서 매겨준다. 최종적으로 sink에 level이 매겨진다면 true, 아닐시 false가 반환된다.  
- DFS Function
